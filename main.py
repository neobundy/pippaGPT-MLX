...
def display_tts_panel():
    ...
    with tts_options:
        ...
        st.session_state.mlx_whisper_model = st.selectbox(
            ...
            index=settings.MLX_WHISPER_MODELS.index(settings.DEFAULT_MLX_WHISPER_MODEL),
        )
        ...
        if st.button("Stop"):
            ...
            if r.status_code == 200:
                ...
                if st.session_state.use_mlx_whisper:
                    ...
                    # OpenAI Whisper
                    st.session_state.transcript = client.audio.transcribe(settings.OPENAI_AUDIO_MODEL, audio_file)["text"]
            else:
